...
contract Whale is ERC20Burnable, Operator {
    ...
    // Initial distribution for the first 2 days genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 30000 ether;
    // Initial distribution for the day 2-5 MAIN-WFTM LP -> MAIN pool
    uint256 public constant INITIAL_MAIN_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the MAIN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("WHALE", "WHALE") {
        // Mints 1 MAIN to contract creator for initial pool setup
    ...
    function _getMainPrice() internal view returns (uint256 _mainPrice) {
        try IOracle(mainOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Main: failed to fetch MAIN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _mainPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_mainPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMainOracle(address _mainOracle) public onlyOperatorOrTaxOffice {
        require(_mainOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MAIN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MAIN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMainPrice = _getMainPrice();
            currentTaxRate = _updateTaxRate(currentMainPrice);
            if (currentMainPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_mainPool != address(0), "!_mainPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_mainPool, INITIAL_MAIN_POOL_DISTRIBUTION);
