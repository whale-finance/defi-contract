...
contract WhaleRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. MAINs to distribute in the pool.
        uint256 lastRewardTime; // Last time that MAINs distribution occurred.
        uint256 accMainPerShare; // Accumulated MAINs per share, times 1e18. See below.
    ...
    uint256[3] public epochMainPerSecond;
    ...
    constructor(address _main, uint256 _poolStartTime) public {
        ...
        epochMainPerSecond[0] = epochTotalRewards[0].div(4 days);
        epochMainPerSecond[1] = epochTotalRewards[1].div(5 days);

        epochMainPerSecond[2] = 0;
        operator = msg.sender;
    }

    modifier onlyOperator() {
        require(operator == msg.sender, "MainRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "MainRewardPool: existing pool?");
        }
    }
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({token: _token, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accMainPerShare: 0, isStarted: _isStarted}));
        if (_isStarted) {
        ...
        for (uint8 epochId = 2; epochId >= 1; --epochId) {
            if (_toTime >= epochEndTimes[epochId - 1]) {
                if (_fromTime >= epochEndTimes[epochId - 1]) {
                    return _toTime.sub(_fromTime).mul(epochMainPerSecond[epochId]);
                }

                uint256 _generatedReward = _toTime.sub(epochEndTimes[epochId - 1]).mul(epochMainPerSecond[epochId]);
                if (epochId == 1) {
                    return _generatedReward.add(epochEndTimes[0].sub(_fromTime).mul(epochMainPerSecond[0]));
                }
                for (epochId = epochId - 1; epochId >= 1; --epochId) {
                    if (_fromTime >= epochEndTimes[epochId - 1]) {
                        return _generatedReward.add(epochEndTimes[epochId].sub(_fromTime).mul(epochMainPerSecond[epochId]));
                    }
                    _generatedReward = _generatedReward.add(epochEndTimes[epochId].sub(epochEndTimes[epochId - 1]).mul(epochMainPerSecond[epochId]));
                }
                return _generatedReward.add(epochEndTimes[0].sub(_fromTime).mul(epochMainPerSecond[0]));
            }
        }
        return _toTime.sub(_fromTime).mul(epochMainPerSecond[0]);
    }

    // View function to see pending MAINs on frontend.
    ...
    function safeMainTransfer(address _to, uint256 _amount) internal {
        uint256 _mainBal = main.balanceOf(address(this));
        if (_mainBal > 0) {
            if (_amount > _mainBal) {
                main.safeTransfer(_to, _mainBal);
            } else {
